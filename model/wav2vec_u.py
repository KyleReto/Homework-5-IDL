...
class Discriminator(nn.Module):
    def __init__(self, dim, cfg: Wav2vec_UConfig):
        ...
        # TODONE: Write a nn.Sequential block of 
        # (*emb_net, dropout of discriminator_dropout, and *inner_net)
        self.net = nn.Sequential(*emb_net,
                                nn.Dropout(cfg.discriminator_dropout),
                                *inner_net)
...
class Generator(nn.Module):
    def __init__(self, input_dim, output_dim, cfg: Wav2vec_UConfig):
        ...
        self.proj = nn.Sequential(
            TransposeLast(),
            # TODONE: Call nn.Conv1d with parameters input_dim, output_dim, 
            # take the generator kernel size, stride, dilation, bias 
            # and padding
            nn.Conv1d(input_dim, output_dim, cfg.generator_kernel,
...
class Wav2vec_U(nn.Module):
    ...
    def __init__(self, target_dict, cfg: Wav2vec_UConfig=Wav2vec_UConfig):
        ...
        # TODONE: Call the Discriminator class with output_size and config
        self.discriminator = Discriminator(output_size, cfg=cfg)

        self.pca_A = self.pca_b = None
        d = cfg.input_dim

        
        # TODONE: Call the JoinSegmenter class with the segmentation from config
        self.segmenter = JoinSegmenter(cfg=cfg.segmentation) 

        # TODONE: Call the Generator class with d, output_size and config
        self.generator = Generator(d, output_size, cfg=cfg)

        self.max_temp, self.min_temp, self.temp_decay = cfg.temp
        self.curr_temp = self.max_temp
        self.update_num = 0

        # Augmentation code, not from fairseq
        self.augmentations = nn.Sequential(
            PermuteBlock(),
            torchaudio.transforms.TimeMasking(50),
            torchaudio.transforms.FrequencyMasking(200),
            PermuteBlock()
        )
  
    def calc_gradient_penalty(self, real_data, fake_data):
        # TODONE: implement gradient penalty
        # Based on fairseq's implementation
        # Trim off data so that sizes match
        sizes = (min(real_data.shape[0], fake_data.shape[0]), min(real_data.shape[1], fake_data.shape[1]))
        real_data = real_data[:sizes[0], :sizes[1]]
        fake_data = fake_data[:sizes[0], :sizes[1]]
        device = real_data.get_device()
        # Probabilistic grad slicing is disabled, so I didn't implement it here.
            # I did test what would happen if I did implement it, and the answer is nothing.
        # Create a mask with which to choose random batches
            # Originally this was done in numpy,
            # but I've switched to torch operations to better match fairseq
        mask = torch.rand(sizes[0], 1, 1)
        mask = mask.expand(real_data.shape)
        mask = mask.to(device)
        # Use said mask to create a linear combination of the two sources
        combined_data = (mask*real_data) + ((1-mask)*fake_data)
        # Run the interpolated vector through the model
        output = self.discriminator(combined_data, None)
        #print(f'Input size: {combined_data.shape}') # 160, 187, 64
        #print(f'Output size: {output.shape}') # 160
        #print(f'Grad Size: {torch.ones(output.shape, device=device).shape}') # 160, 187, 64
        # Use autograd to get the gradients of that run
        ...
        # Get the 2 norm of gradients
        norm = gradients.norm(2, dim=1)
        # subtract 1 from that & square
        penalty = (norm-1)**2
        # Return that.
        return penalty
    ...
    def normalize(self, dense_x):
        ...
        #TODONE: Call torch.mean on hard_x after converting it to float and use dim == 0 
    ...
    def forward(
        ...
        # TODONE: if self.training run augmentations on features
        if self.training:
        ...
        if d_step:
            ...
            if self.training and self.gradient_penalty > 0:
                #TODONE: Call the calc_gradient_penalty function you implemented with token_x and dense_x
            ...
            if self.smoothness_weight > 0:
                # Based on fairseq code
                # TODONE: implement smoothness loss
                # Get the output at the current and next segments.
                    # (really, we're using the output and trimming off the end and start,
                    # but if you subtract these from one another it's equivalent but vectorized)
                curr_p = dense_logits[:, :-1]
                next_p = dense_logits[:, 1:]
                # Get the difference
                ###diff = (curr_p - next_p)
                # norm that
                ###norm = torch.linalg.norm(diff, ord=2, dim=1)
                # square that
                ###square = norm ** 2
                # Get the MSE loss, which does all that for us.
                smoothness_loss = F.mse_loss(
                    curr_p, next_p, reduction="none"
                )
                # Remove (zero out) meaningless padding data
                smoothness_loss[dense_padding_mask[:, 1:]] = 0
                # avg over all generated segments
                smoothness_loss = torch.mean(smoothness_loss) * self.smoothness_weight * sample_size
